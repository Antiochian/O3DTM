function data = generate_crater_ray_tracing(crater_name, use_seasons)
% GENERATE_CRATER_RAY_TRACING performs ray tracing for 3D crater
% environment used to speed up 3D model
%
% data = generate_crater_ray_tracing(crater_name) generates, saves and
% returns ray tracing data file for environment crater_name for both no
% seasons and seasons
%
% data = generate_crater_ray_tracing(crater_name, use_seasons) defines if
% seasonal variation calculations should be performed

if nargin == 0 || strcmp(crater_name, 'all')
    generate_crater_ray_tracing('bruce', false);
    generate_crater_ray_tracing('blagg', false);
    generate_crater_ray_tracing('erlanger_compressed', false);
    generate_crater_ray_tracing('bruce', true);
    generate_crater_ray_tracing('blagg', true);
    generate_crater_ray_tracing('erlanger_compressed', true);
    return
end
if strcmp(crater_name, 'seasons')
    ...
    for crater_idx = 1:numel(crater_name_arr)
        ...
        generate_crater_ray_tracing(crater_name);
    end
    for crater_idx = 1:numel(crater_name_arr)
        crater_name = crater_name_arr{crater_idx};
...
if strcmp(crater_name(1:3), 'ls_')
    for ls_idx = flip(1:8)
        generate_crater_ray_tracing(sprintf('ls%d%s', ls_idx, crater_name(3:end)), false);
        generate_crater_ray_tracing(sprintf('ls%d%s', ls_idx, crater_name(3:end)), true);        
    end
    return
end
if nargin < 2
...
for lat_idx = 1:numel(lat_arr)
    ...
    for long_idx = 1:numel(long_arr)
        ...
        t_calculation_step_length = 1e3; % calculate 1000 time steps at a time using parfor loop
        t_calculation_steps = ceil(t_steps/t_calculation_step_length);
        
        % use single precision values to save memory and disk space when
        % theta values are saved later
        ...
        theta_3dmat(lat_idx,long_idx,:) = theta_arr(1:t_steps); % save values for current pixel
...
clear data
summarise_ray_tracing(target_name);
end